...
void ScriptManager::runScript(std::string functionName, int arg_count, ...)
{
	if (this->scriptLoaded)
	{
		...
		{
			// Use variant implementation for args
			...
			dukglue_push(this->ctx, va_arg(args, PlayerObject*));
		}
		va_end(args);
...
void ScriptManager::runEventScript(std::string functionName, int arg_count, ...)
{
	if (this->scriptLoaded)
	{
		// This indicates the name of the function within the script file that you wish to run
