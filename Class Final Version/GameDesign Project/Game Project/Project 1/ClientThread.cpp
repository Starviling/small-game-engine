...
void ClientThread::run()
{
			...
			if (!EventManager::getReplaying()) {
				...
				if (socket.recv(&request, ZMQ_DONTWAIT)) {
					...
					json jInfo;
					{
						...
						/*
						AbstractGameObject* newPlayer = new PlayerObject(0, 0, gameTimeFrame, xyTranslation, deleteList);
						newPlayer->setIndex(objectList->size());
						objectList->push_back(newPlayer);
						*/
						std::cout << "Client Generated." << std::endl;
	...
	//					std::cout << "Player ID: " + jInfo.at(0) << std::endl;
						...
						*/
						std::cout << "Client position: " << jInfo.at(1)["position"] << std::endl;

						zmq::message_t reply(15);
						memcpy(reply.data(), "Client Updated.", 15);
