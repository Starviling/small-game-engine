...
*/
PlayerObject::PlayerObject(float xloc, float yloc, long double* gameTimeFrame, sf::Vector2f* xyTranslation, std::vector<int>* deleteList)
...
PlayerObject::PlayerObject(float xloc, float yloc, long double* framePeriod, long double* gameTimeFrame, bool* focused, std::vector<AbstractGameObject*>* objectList,
	sf::Vector2f* xyTranslation, std::vector<int>* deleteList)
	:AbstractGameObject(framePeriod, gameTimeFrame, focused, true, xyTranslation, objectList, deleteList)
{
	this->pModel = new PlayerModel(xloc, yloc);
	...
	this->threadUnsafeList.push_back(new PlayerControl(this));
