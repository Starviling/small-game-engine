...
int main()
{
    ...
    for (int i = 1; i < entitiesNumber + 1; i++) {
        ...
        if (jInfo.at(i)["id"] == playerId) {
            ...
            PlayerObject* player = new PlayerObject(0, 0, &gameTimeFrame, &xyTranslation);
            player->serverUpdate(jInfo.at(i));
            objectList.push_back(player);
        }
        else if (jInfo.at(i)["id"] == platformId) {
            std::cout << "platform added" << std::endl;
            PlatformObject* platform = new PlatformObject(&gameTimeFrame, &xyTranslation);
            platform->serverUpdate(jInfo.at(i));
            objectList.push_back(platform);
        }
        else if (jInfo.at(i)["id"] == deathId) {
            std::cout << "platform added" << std::endl;
            DeathZoneObject* zone = new DeathZoneObject(&gameTimeFrame, &xyTranslation);
            zone->serverUpdate(jInfo.at(i));
            objectList.push_back(zone);
        }
        else if (jInfo.at(i)["id"] == spawnId) {
            std::cout << "platform added" << std::endl;
            SpawnZoneObject* zone = new SpawnZoneObject(&gameTimeFrame, &xyTranslation);
            zone->serverUpdate(jInfo.at(i));
            objectList.push_back(zone);
        }
        else if (jInfo.at(i)["id"] == translateId) {
            std::cout << "platform added" << std::endl;
            TranslateZoneObject* zone = new TranslateZoneObject(&gameTimeFrame, &xyTranslation);
            zone->serverUpdate(jInfo.at(i));
            objectList.push_back(zone);
        }
    }
    ...
    while (window.isOpen()) {
...

        {
            ...
            // Handle incase there are now more platforms
